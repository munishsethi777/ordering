  ...
  grid = {
    ...
    alter: function (action, index, amount, source, keepEmptyRows) {
      ...
      switch (action) {
        ...
        //mathan
        var rlen = datamap.instance.countRows();
        var rows = datamap.instance.getSelected();
        if(   rows != undefined
            && rows[0] != (rlen - 6)
            && rows[0] != (rlen - 5)
            && rows[0] != (rlen - 4)
    ...
    adjustRowsAndCols: function () {
      ...
      //if (emptyRows < priv.settings.minSpareRows) {
        for (; emptyRows < 3 && instance.countRows() < priv.settings.maxRows; emptyRows++) {
          datamap.createRow(instance.countRows()-7, 1, true);
        }
      //}
      }
  ...
  Handsontable.DataMap.prototype.createRow = function (index, amount, createdAutomatically) {
    ...
    while (numberOfCreatedRows < amount && this.instance.countRows() < maxRows) {
      ...
      if (index === this.instance.countRows()) {
        ...
        this.dataSource.push(row); //this.dataSource.splice(index, 0, row);
      }
